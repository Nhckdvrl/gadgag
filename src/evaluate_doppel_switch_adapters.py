...
def score(model,tok,xs,fold,cond):
 ...
 for x in [z for z in xs if z['group']=='false_friend']:
  ...
  mz=m(x['train_zh']);mj=m(x['eval_ja']);out.append({'id':x['id'],'fold':fold,'condition':cond,'treated':x['treated_fold']==fold,'margin_zh_for_ja':mz,'margin_ja_for_ja':mj,'diagonal_midpoint':(mj+mz)/2,'sense_switch':mj-mz})
